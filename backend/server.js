...
async function normalizeDeliveryBody(db, body, files = {}, user, existing = {}) {
  ...
  const data = {
    ...
    cargo_name: cleanString(body.cargo_name, existing.cargo_name || ''),
    loading_weight_kg: round2(toNumber(body.loading_weight_kg, toNumber(existing.loading_weight_kg, 0))),
    unloading_weight_kg: round2(toNumber(body.unloading_weight_kg, toNumber(existing.unloading_weight_kg, 0))),
    ...
    trip_fee_baht: round2(toNumber(body.trip_fee_baht, toNumber(existing.trip_fee_baht, 0))),
    allowance_baht: round2(toNumber(body.allowance_baht, toNumber(existing.allowance_baht, 0))),
    other_income_baht: round2(toNumber(body.other_income_baht, toNumber(existing.other_income_baht, 0))),
    total_income_baht: 0,
...
async function enrichDelivery(db, delivery) {
  ...
  d.decimal_fix_version = 'v52_close_job_summary';
  return d;
}
