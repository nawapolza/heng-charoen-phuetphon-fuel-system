        ...
        <div className="divide-y divide-slate-100">
          {transactions.map((row) => (
            <div key={row.id || `${row.created_at}-${row.item_type}`} className="grid gap-2 p-4 text-sm md:grid-cols-7 md:items-center">
              ...
              <div className={Number(row.change_liters || row.quantity_liters) >= 0 ? 'font-black text-blue-700' : 'font-black text-red-700'}>{number(row.change_liters ?? row.quantity_liters, 2)} ลิตร</div>
