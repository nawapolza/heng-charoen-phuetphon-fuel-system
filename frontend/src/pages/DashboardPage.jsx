  ...
  return (
    <div className="page-shell dashboard-page">
      ...
      <section className="dashboard-metrics">
        <Metric icon={ClipboardList} label="รายการทั้งหมด" value={number(data?.total_trips)} unit="รายการ" helper="งานในช่วงที่เลือก" />
        ...
        <div className="dashboard-table-scroll">
          ...
          {(data?.latest || []).map((row) => (
            <div key={row.id} className="dashboard-table dashboard-table-row">
              ...
              <span data-label="ปลายทาง">{row.destination_place || '-'}</span>
            </div>
          ))}
      ...
      {rows.map((item, index) => (
        <div key={`${item.name}-${index}`} className="rank-row">
          ...
          <div className="rank-content">
            <div><strong>{item.name}</strong><span>{number(item.trips)} รายการ</span></div>
