        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_.75fr] xl:items-end">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-2xl font-black tracking-tight md:text-4xl">ภาพรวมเฮงเจริญพืชผล</h1>
            ...
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-emerald-100">สรุปงานน้ำมัน รถ สต๊อก และประสิทธิภาพการวิ่งในหน้าจอเดียว เลือกช่วงวันที่และติดตามข้อมูลล่าสุดได้ทันที</p>
            <div className="mt-4 flex flex-wrap gap-2 text-xs font-black text-emerald-50">
          ...
          <div className="rounded-[1.5rem] border border-white/15 bg-white/10 p-3 backdrop-blur">
            <p className="mb-2 flex items-center gap-2 text-sm font-black text-emerald-50"><CalendarDays size={16} /> เลือกช่วงวันที่ดูกราฟ</p>
          ...
          return (
            <div key={stock.item_type} className="card-clean p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="rounded-3xl bg-emerald-50 p-3 text-emerald-700"><Boxes size={24} /></div>
              </div>
              <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${balance < 100 ? 'bg-red-500' : balance < 300 ? 'bg-amber-500' : 'bg-emerald-600'}`} style={{ width: `${Math.max(8, Math.min(100, balance / 10))}%` }} /></div>
...
function PresetButton({ active, onClick, children }) {
  return <button type="button" onClick={onClick} className={`rounded-2xl px-3 py-2 text-sm font-black transition ${active ? 'bg-white text-emerald-700 shadow-lg' : 'bg-white/10 text-white hover:bg-white/20'}`}>{children}</button>;
}

function Metric({ icon: Icon, label, value, helper, soft = 'blue' }) {
  const tone = { blue: 'bg-emerald-50 text-emerald-700', emerald: 'bg-emerald-50 text-emerald-700', amber: 'bg-amber-50 text-amber-700', red: 'bg-red-50 text-red-700' }[soft] || 'bg-emerald-50 text-emerald-700';
...
function DailyChart({ rows }) {
  ...
  return <div className="overflow-x-auto pb-2"><div className="flex min-w-[560px] items-end gap-2 rounded-3xl bg-slate-50 p-4">{rows.map((row) => { const height = Math.max(14, (Number(row.value || 0) / max) * 150); return <div key={row.name} className="flex flex-1 flex-col items-center justify-end gap-2"><div className="text-[10px] font-black text-slate-400">{number(row.value, 0)}</div><div className="w-full rounded-t-2xl bg-gradient-to-t from-emerald-600 to-amber-400 shadow-lg shadow-emerald-600/10" style={{ height }} title={`${row.name}: ${number(row.value, 2)} ลิตร`} /><div className="w-12 truncate text-center text-[10px] font-black text-slate-500">{String(row.name).slice(5)}</div></div>; })}</div></div>;
}

function HorizontalBars({ rows, total, suffix }) {
  if (!rows.length) return <Empty text="ยังไม่มีข้อมูลประเภทน้ำมัน" />;
  const safeTotal = Math.max(Number(total || 0), 1);
  return <div className="space-y-4">{rows.map((item) => <div key={item.name}><div className="mb-2 flex justify-between gap-3 text-sm font-black"><span>{item.name}</span><span>{number(item.value, 2)} {suffix}</span></div><div className="h-4 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-emerald-600 to-amber-400" style={{ width: `${Math.min(100, (Number(item.value || 0) / safeTotal) * 100)}%` }} /></div><p className="mt-1 text-xs font-bold text-slate-400">{number((Number(item.value || 0) / safeTotal) * 100, 1)}%</p></div>)}</div>;
}

function RankList({ rows, suffix }) {
  if (!rows.length) return <Empty text="ยังไม่มีข้อมูล" />;
  const max = Math.max(...rows.map((r) => Number(r.value || 0)), 1);
  return <div className="space-y-2">{rows.map((item, index) => <div key={`${item.name}-${index}`} className="rounded-3xl bg-slate-50 px-4 py-3 text-sm"><div className="mb-2 flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate font-black text-slate-800">{index + 1}. {item.name}</p><p className="text-xs font-bold text-slate-400">{number(item.trips)} รายการ</p></div><span className="shrink-0 font-black text-emerald-700">{number(item.value, 2)} {suffix}</span></div><div className="h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${Math.max(5, (Number(item.value || 0) / max) * 100)}%` }} /></div></div>)}</div>;
}
