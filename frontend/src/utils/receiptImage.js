...
function drawSummaryCard(ctx, x, y, w, h, label, value, tone = 'slate') {
  const tones = {
    slate: ['#fbfaf5', '#e6e1d5', '#292923', '#8c887c'],
    blue: ['#fff7df', '#f2d58a', '#6f4b0e', '#9b8c6e'],
    green: ['#eef8f1', '#c7e4d0', '#0e4d35', '#789184'],
  };
...
export async function createReceiptImageBlob(row = {}) {
  ...
  bg.addColorStop(0, '#f7f2df');
  bg.addColorStop(0.44, '#f7f6ef');
  ...
  head.addColorStop(1, '#e7f2e8');
  ctx.fillStyle = head;
  roundedRect(ctx, 42, 42, 996, 390, 58);
  ctx.fill();

  try {
    const logo = await loadImage('/logo-heng.svg');
    ctx.fillStyle = '#ffffff';
    ...
    ctx.fillStyle = '#145c3d';
    roundedRect(ctx, 90, 94, 150, 150, 34);
    ctx.fill();
    drawText(ctx, 'HENG', 108, 145, 120, { size: 38, weight: 950, color: '#fff', maxLines: 1 });
  }

  drawText(ctx, 'HENG CHAROEN PHUETPHON', 280, 92, 460, { size: 28, weight: 950, color: '#145c3d', maxLines: 1 });
  ...
  drawPill(ctx, 418, 352, safeText(row.item_type, '-'), '#fff3cf', '#7b520f', 130);
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    ctx.fillStyle = i < photoCount ? '#fff3cf' : '#f8f6ef';
    roundedRect(ctx, bx, y + 34, 70, 70, 18);
    ctx.fill();
    ctx.strokeStyle = i < photoCount ? '#efd28a' : '#e5e0d5';
    ctx.stroke();
    drawText(ctx, i < photoCount ? String(i + 1) : '-', bx + 25, y + 50, 30, { size: 28, weight: 950, color: i < photoCount ? '#7b520f' : '#c8c2b6', maxLines: 1 });
  }

  drawText(ctx, 'สร้างจากระบบจัดการน้ำมัน เฮงเจริญพืชผล', 90, 1630, 580, { size: 26, weight: 850, color: '#64748b', maxLines: 1 });
...
function receiptFileName(row = {}) {
  return `HENG-${safeText(row.plate_no, 'receipt')}-${Date.now()}.png`.replace(/[\\/:*?"<>|\s]+/g, '-');
}
...
export async function saveReceiptImageToDevice(row = {}, options = {}) {
  ...
  if (preferShare && navigator?.canShare?.({ files: [file] }) && navigator?.share) {
    await navigator.share({ title: 'ใบสรุปน้ำมัน เฮงเจริญพืชผล', text: 'ใบสรุปน้ำมันสำหรับส่งเจ้าของกิจการ', files: [file] });
