          ...
          <div className="min-w-0">
            ...
            <div className="mt-3 rounded-[1.35rem] border border-blue-100 bg-white/85 p-3 shadow-sm ring-1 ring-blue-50">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                <RouteStepBadge mode="origin" value={row.origin_place} />
                <RouteStepBadge mode="destination" value={row.destination_place} />
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-3 xl:grid-cols-2">
          <section className="overflow-hidden rounded-[1.35rem] border border-sky-100 bg-sky-50/70">
            ...
            <div className="divide-y divide-sky-100 px-3 py-1">
              <CloseJobLine icon={MapPin} prefix="ขึ้นงาน" label={row.origin_place || 'จุดรับสินค้า / บ่อต้นทาง'} value={kgText(row.loading_weight_kg)} />
              <CloseJobLine icon={MapPin} prefix="ลงงาน" label={row.destination_place || 'จุดลงงาน / ปลายทาง'} value={kgText(row.unloading_weight_kg)} />
            </div>
          </section>
          ...
          <section className="overflow-hidden rounded-[1.35rem] border border-emerald-100 bg-emerald-50/70">
            <div className="flex items-center gap-2 border-b border-emerald-100 bg-white/75 px-4 py-3">
              ...
              <div>
                <p className="text-sm font-black text-slate-900">สรุปรายได้เที่ยวนี้</p>
        ...
        <details className="group rounded-[1.35rem] border border-slate-200 bg-white">
          ...
          <div className="space-y-4 border-t border-slate-100 p-4">
            <div className="field-grid">
              ...
              <Info label="ประเภทสินค้า / ชื่องาน" value={row.cargo_name || '-'} />
              <Info label="จุดรับสินค้า / บ่อต้นทาง" value={row.origin_place || '-'} />
              <Info label="น้ำหนักต้นทาง" value={kgText(row.loading_weight_kg)} />
              <Info label="จุดลงงาน / ปลายทาง" value={row.destination_place || '-'} />
              <Info label="น้ำหนักปลายทาง" value={kgText(row.unloading_weight_kg)} />
              ...
              <Info label="ผู้จ่ายค่าแรง" value={row.wage_payer || '-'} />
              <Info label="สถานะรายได้" value={row.payment_status === 'paid' ? 'จ่ายแล้ว' : 'รอจ่าย / ไม่ระบุ'} />
            </div>
