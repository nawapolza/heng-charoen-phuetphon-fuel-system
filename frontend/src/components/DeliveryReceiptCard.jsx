...
export default function DeliveryReceiptCard({ row, onEdit, onDelete }) {
  ...
  return (
    <article className="receipt-card app-receipt-card overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-[0_12px_38px_rgba(15,23,42,.06)]">
      <div className="border-b border-slate-100 bg-gradient-to-r from-white via-slate-50 to-emerald-50/50 p-4 md:p-5">
        <div className="receipt-capture-banner mb-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[11px] font-black uppercase tracking-[.16em] text-emerald-700">เฮงเจริญพืชผล</p>
        ...
        <details className="group rounded-[1.35rem] border border-slate-200 bg-white">
          ...
          <div className="space-y-4 border-t border-slate-100 p-4">
            <div className="field-grid">
              ...
              <Info label="น้ำหนักไม้สับ" value={`${number(row.cargo_sand_weight, 2)} ตัน`} />
            ...
            <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50/70 p-3 md:p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h4 className="flex items-center gap-2 text-sm font-black text-slate-800"><Camera size={17} className="text-emerald-600" /> รูปภาพแนบแยกตามหมวด</h4>
...
function SummaryInfo({ label, value, tone = 'slate' }) {
  const cls = {
    ...
    blue: 'border-emerald-100 bg-emerald-50 text-emerald-950',
...
function MiniLine({ icon: Icon, label, value }) {
  return <div className="flex items-center gap-2 rounded-2xl bg-white px-3 py-2"><Icon size={16} className="shrink-0 text-emerald-600" /><div className="min-w-0"><p className="truncate text-[11px] font-black text-slate-400">{label}</p><p className="truncate text-sm font-black text-slate-800">{value}</p></div></div>;
}
...
function PhotoThumb({ path, index, small = false }) {
  ...
  if (isPdf(path)) {
    return (
      <a href={href} target="_blank" rel="noreferrer" className={`group relative flex ${sizeClass} items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100`}>
