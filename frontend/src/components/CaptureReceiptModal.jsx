      ...
      <div className="capture-modal-panel max-h-[94dvh] w-full max-w-[560px] overflow-y-auto rounded-t-[2rem] bg-slate-100 p-3 shadow-2xl sm:rounded-[2rem] sm:p-4">
        <div className="mb-3 flex items-center justify-between gap-2 px-1 print:hidden">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button type="button" onClick={saveReceiptImage} disabled={savingImage} className="hidden rounded-2xl bg-emerald-600 px-3 py-2 text-xs font-black text-white shadow-lg shadow-emerald-600/20 sm:inline-flex">
        ...
        <article className="capture-receipt rounded-[1.75rem] border border-emerald-100 bg-white shadow-[0_18px_50px_rgba(15,23,42,.16)]">
          <div className="rounded-t-[1.75rem] bg-gradient-to-br from-white via-amber-50 to-emerald-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-black uppercase tracking-[.2em] text-emerald-700">เฮงเจริญพืชผล</p>
          ...
          <div className="space-y-3 p-3 sm:p-4">
            ...
            <div className="flex flex-col gap-2 rounded-[1.35rem] border border-emerald-100 bg-emerald-50/70 p-3 text-xs font-bold leading-5 text-emerald-900 print:hidden">
...
function CaptureThumb({ path, index }) {
  ...
  if (isPdf(path)) {
    return (
      <a href={href} target="_blank" rel="noreferrer" className="flex aspect-square items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100">
        <FileText size={22} />
      </a>
    );
  }
